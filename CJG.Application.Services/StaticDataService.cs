...
{
	/// <summary>
	/// <typeparamref name="StaticDataService"/> class, provides a way to fetch and cache various lookup collections.
	/// </summary>
	public class StaticDataService : Service, IStaticDataService
	{
		...
		{
			return _GetAll<TEntity>(() => _dbContext.Set<TEntity>()
				.AsNoTracking()
		...
		{
			return _GetAll<Country>(() => _dbContext.Set<Country>()
				.OrderBy(x => x.RowSequence)
				.ThenBy(x => x.Name))
				.Select(r => (Country)r)
				.Where(r => r.IsActive);
		...
		{
			return _GetAll<Region>(() => _dbContext.Set<Region>()
				.Include(r => r.Country)
				.OrderBy(x => x.RowSequence)
				.ThenBy(x => x.Name))
				.Select(r => (Region)r)
		...
		public Region GetRegion(string countryId, string id)
		{
			return _dbContext.Set<Region>()
				...
				.OrderBy(x => x.RowSequence)
				.ThenBy(x => x.Name)
				.FirstOrDefault(r => r.CountryId == countryId && r.Id == id);
		}
		...
		/// </summary>
		/// <returns></returns>
		public IEnumerable<TrainingProviderType> GetTrainingProviderTypes(TrainingProviderTypeUsage usage)
		{
			var trainingProviderTypes = Get<TrainingProviderType, int>().Where(e => e.IsActive);

			if (usage != TrainingProviderTypeUsage.All)
				return trainingProviderTypes
					.Where(e => e.UseForProviderTypes == TrainingProviderTypeUsage.All || e.UseForProviderTypes == usage);
		...
		/// </summary>
		/// <returns></returns>
		public IEnumerable<FiscalYear> GetFiscalYears(int? limitEndYearRange = null)
		{
			if (!limitEndYearRange.HasValue)
				return Get<FiscalYear>()
					.OrderBy(fy => fy.StartDate)
					.ThenBy(fy => fy.Caption);

			var endDate = AppDateTime.UtcNow.AddYears(limitEndYearRange.Value);
			return Get<FiscalYear>()
				.Where(fy => fy.StartDate < endDate)
				.OrderBy(fy => fy.StartDate)
				.ThenBy(fy => fy.Caption);
