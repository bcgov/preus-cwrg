...
namespace CJG.Application.Services
{
	public static class HttpPostedFileExtensions
	{
		...
		public static void CreateNewVersionForNote(this Attachment attachment, HttpPostedFileBase file, string description)
		{
			var result = file.UploadFile(description, permittedFileTypesKey: "NotesPermittedAttachmentTypes");
			attachment.CreateNewVersion(result.FileName, result.Description, result.FileExtension, result.AttachmentData);
		}
		...
		private static bool IsValidFileType(MemoryStream memoryStream, string fileExtension)
		{
			...
			using (BinaryReader reader = new BinaryReader(memoryStream))
			{
				...
				var bypassFileTypes = new[] { ".eml", ".msg", ".pst", ".xlsx", ".xls", ".doc", ".docx" };
				...
				if (reader.BaseStream.Length >= 16)
				{
					...
					{
						var otherAllowedFileTypes = new[] { ".jpg", ".jpeg" };
						isValidFileType = otherAllowedFileTypes.Contains(fileExtension, StringComparer.InvariantCultureIgnoreCase);
					}
				}
			}
