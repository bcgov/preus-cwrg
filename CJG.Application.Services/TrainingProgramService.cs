...
namespace CJG.Application.Services
{
	...
	public class TrainingProgramService : Service, ITrainingProgramService
	{
		...
		public IEnumerable<string> GetTrainingProgramTitles(int fiscalYearId)
		{
			var defaultGrantProgramId = GetDefaultGrantProgramId();
			var trainingProgramTitles = _dbContext.GrantApplications
				.AsNoTracking()
				.Where(ga => ga.GrantOpening.TrainingPeriod.FiscalYearId == fiscalYearId)
				...
				.SelectMany(x => x.TrainingPrograms)
				.Select(tp => tp.CourseTitle)
				.Distinct()
				.OrderBy(tp => tp);

			return trainingProgramTitles;
		}
