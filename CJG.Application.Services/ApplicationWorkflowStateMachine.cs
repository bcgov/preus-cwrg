...
namespace CJG.Application.Services
{
	...
	public class ApplicationWorkflowStateMachine : Service
	{
		...
		private readonly StateMachine<ApplicationStateInternal, ApplicationWorkflowTrigger>.TriggerWithParameters<string>
			_returnUnderAssessmentToDraftTrigger;

		private readonly StateMachine<ApplicationStateInternal, ApplicationWorkflowTrigger>.TriggerWithParameters<string>
			_recommendForDenialTrigger;

		private readonly StateMachine<ApplicationStateInternal, ApplicationWorkflowTrigger>.TriggerWithParameters<string>
			_editRecommendForDenialReasonTrigger;
			...
			ILogger logger) : base(dbContext, httpContext, logger)
		{
			...
			_returnUnderAssessmentToDraftTrigger = _stateMachine.SetTriggerParameters<string>(ApplicationWorkflowTrigger.ReturnUnderAssessmentToDraft);
			_recommendForDenialTrigger = _stateMachine.SetTriggerParameters<string>(ApplicationWorkflowTrigger.RecommendForDenial);
			_editRecommendForDenialReasonTrigger = _stateMachine.SetTriggerParameters<string>(ApplicationWorkflowTrigger.EditRecommendForDenialReasons);
		...
		private void ConfigureStateTransitions()
		{
			_stateMachine.Configure(ApplicationStateInternal.Draft)
				.OnEntryFrom(_returnUnderAssessmentToDraftTrigger, reason => OnReturnUnderAssessmentToDraft(reason))
			...
			_stateMachine.Configure(ApplicationStateInternal.RecommendedForDenial)
				...
				.OnEntryFrom(_editRecommendForDenialReasonTrigger, reason => OnEditRecommendForDenialReason(reason))
				.Permits(_grantApplication.ApplicationStateInternal);
		...
		private void OnSubmitApplication()
		{
			try
			{
				if (_grantApplication.IsSubmittable())
				{
					...
					_grantApplication.ReturnedToDraft = null;
		...
		/// </summary>
		private void OnReturnUnderAssessmentToDraft(string reason = null)
		{
			_grantApplication.ApplicationStateExternal = ApplicationStateExternal.Incomplete;
			_grantApplication.DateSubmitted = null;
			_grantApplication.ReturnedToDraft = AppDateTime.UtcNow;
			...
			LogStateChanges(noteReason: reason, stateChangeReason: reason);
		...
		private void OnEditRecommendForDenialReason(string response)
		{
			if (string.IsNullOrEmpty(response))
				LogStateChanges(response, response);
			else
			...
			_logger.Info($"Grant application {_grantApplication.Id} has been recommended for denial reason updated.");
		}
		...
		private void OnAcceptGrantAgreement()
		{
			try
			{
				...
				_grantApplication.GrantAgreement.DateAccepted = agreementDate;

				var generateNewDocuments = false;
				// Move the Start Date forward to the agreement date if it is past the original start date to comply with legal requirements
				if (agreementDate > _grantApplication.GrantAgreement.StartDate)
				{ 
					_grantApplication.GrantAgreement.StartDate = agreementDate;
					generateNewDocuments = true;
				}

				if (agreementDate > _grantApplication.StartDate)
				{
					_grantApplication.GrantAgreement.StartDate = agreementDate;
					_grantApplication.StartDate = agreementDate;
					generateNewDocuments = true;
				}
...
Thank you.";
		...
		public void EditRecommendForDenialReason(string response)
		{
			CanPerformTrigger(ApplicationWorkflowTrigger.EditRecommendForDenialReasons);
			_stateMachine.Fire(_editRecommendForDenialReasonTrigger, response);
		}
		...
		public void ReturnUnderAssessmentToDraft(string reason = null)
		{
			CanPerformTrigger(ApplicationWorkflowTrigger.ReturnUnderAssessmentToDraft);
			_stateMachine.Fire(_returnUnderAssessmentToDraftTrigger, reason);
		}
		...
		private void LogStateChanges(string noteReason = "", string stateChangeReason = "", string suffix = null, bool addReason = true)
		{
			...
			switch (_grantApplication.ApplicationStateInternal)
			{
				...
				case ApplicationStateInternal.RecommendedForDenial:
					var hasSameState = _grantApplication.ApplicationStateInternal == _originalState;
					if (hasSameState)
						noteContent = "Recommended for Denial reason updated.";
					break;
			}
