...
namespace CJG.Application.Services
{
	...
	public class NotificationService : Service, INotificationService
	{
		...
		{
			var query = _dbContext.NotificationQueue
				.Where(nt => nt.GrantApplication.GrantOpening.GrantStream.GrantProgramId == grantProgramId);
		...
		{
			var query = _dbContext.NotificationQueue
				.Where(nt => nt.GrantApplicationId == grantApplicationId &&
		...
		public PageList<NotificationQueue> GetNotifications(int page, int quantity, NotificationFilter filter)
		{
			...
			if (!string.IsNullOrWhiteSpace(filter.Status))
			{
				...
				query = aStatus.Length > 1
					? query.Where(n => aStatus.Contains(n.State.ToString()))
					: query.Where(n => n.State.ToString() == filter.Status);
			}
		...
		{
			return _dbContext.NotificationQueue
				.Where(nt => nt.GrantApplication.GrantOpening.GrantStream.GrantProgramId == grantProgramId)
					...
					foreach (var grantApplication in query.OrderByDescending(g => g.DateUpdated).Skip(skip).Take(take).ToArray())
					{
						foreach (var grantProgramNotificationType in grantProgramNotificationTypes)
						{
							try
							{
								...
								var recipientType = grantProgramNotificationType.NotificationType.RecipientRule;

								if (recipientType == NotificationRecipientRules.AllExitingParticipants)
								{
									// Create a notification for each user
									foreach (var participantForm in GetParticipants(grantApplication.Id).ToArray())
									{
										grantApplication.NotificationQueue.Add(GenerateNotificationMessage(grantApplication, participantForm, grantProgramNotificationType));
										count++;
									}
								}
								else
		...
		public IEnumerable<NotificationQueue> SendScheduledNotifications(DateTime appDate = default)
		{
			...
			var queuedNotifications = _dbContext.NotificationQueue
				.Where(n => n.State != NotificationState.Sent && (n.SendDate == null || n.SendDate <= appDate))
				.ToArray();
		...
		/// <returns></returns>
		public NotificationQueue GenerateNotificationMessage(GrantApplication grantApplication, INotificationRecipient user, GrantProgramNotificationType grantProgramNotificationType)
		{
			if (grantApplication == null)
				throw new ArgumentNullException(nameof(grantApplication));
		...
		/// <param name="notificationType"></param>
		/// <returns></returns>
		public NotificationQueue GenerateNotificationMessage(GrantApplication grantApplication, INotificationRecipient user, NotificationType notificationType)
		{
			if (grantApplication == null)
				throw new ArgumentNullException(nameof(grantApplication));
		...
		public string ValidateModelKeywords(string text, string[] excludedKeywords = null)
		{
			...
			var subjectVariablesKeywords = Regex.Matches(text, regex).Cast<Match>().Select(m => m.Groups[0].Value).Distinct().ToList();
			if (subjectVariablesKeywords.Any(s => s == "@"))
				subjectVariablesKeywords.Remove("@");
		...
		/// <param name="grantApplicationId"></param>
		/// <returns></returns>
		private IQueryable<INotificationRecipient> GetParticipants(int grantApplicationId)
		{
			var participants = _dbContext.ParticipantForms
				.Where(pf => pf.GrantApplicationId == grantApplicationId)
				.Where(pf => pf.IsExcludedFromClaim == false)
				.Where(pf => pf.WithdrawalKey == null)
				.Where(pf => pf.TrainingExitDate == null);
		...
		private static NotificationParticipantReportRules ParticipantReportingRule(GrantApplication grantApplication)
		{
			...
			if (participants == 0)
				return NotificationParticipantReportRules.NoneReported;
		...
		private static NotificationClaimReportRules NotificationClaimReportingRule(GrantApplication grantApplication, NotificationClaimReportRules rule)
		{
			...
			if (rule == NotificationClaimReportRules.ClaimReported && claims.Any())
		...
		private bool ResendRule(GrantApplication grantApplication, NotificationType notificationType, DateTime appDate = default)
		{
			...
			{
				var currentClaim = grantApplication.GetCurrentClaim();
				if (currentClaim != null && notifications.Any(n => n.BatchNumber.Contains($"C:{currentClaim.Id}-CV:{currentClaim.ClaimVersion}")))
					return false;
			}
			...
			switch (resendRule)
			{
				...
				case NotificationResendRules.AgreementDateChanged:
					...
					var agreementUpdatedOn = _dbContext.Notes
						...
						.Where(n => n.GrantApplicationId == grantApplication.Id)
						.Where(n => n.NoteTypeId == NoteTypes.TD)
