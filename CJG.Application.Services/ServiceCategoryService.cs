			...
			foreach (var serviceCategory in serviceCategories)
			{
				...
				else
				{
					...
					foreach (var eet in serviceCategory.EligibleExpenseTypes)
					{
						...
						if (!eet.EligibleCosts.Any(ec => ec.TrainingCost.GrantApplication.ApplicationStateInternal >= ApplicationStateInternal.New
						                                 && ec.TrainingCost.GrantApplication.ApplicationStateInternal != ApplicationStateInternal.ApplicationWithdrawn))
						{
							...
							foreach (var serviceLine in serviceCategory.ServiceLines)
							{
								...
								foreach (var eeb in serviceLine.EligibleExpenseBreakdowns)
								{
									...
									eeb.EnableCustomTitle = serviceLine.EnableCustomTitle;
