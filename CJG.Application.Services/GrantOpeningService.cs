...
namespace CJG.Application.Services
{
	public class GrantOpeningService : Service, IGrantOpeningService
	{
		...
		/// <summary>
		/// Get the 'current' Grant Opening for an existing Grant Opening. Used when Cloning an application and getting a newer Grant Opening.
		/// </summary>
		/// <param name="grantOpeningId"></param>
		/// <returns></returns>
		public GrantOpening GetCurrentGrantOpening(int grantOpeningId)
		{
			var now = AppDateTime.UtcNow;

			var grantOpening = Get(grantOpeningId);
			if (grantOpening == null)
				return null;

			var openings = GetGrantOpenings(now);
			var currentOpening = openings?.FirstOrDefault(go => go.GrantStreamId == grantOpening.GrantStreamId);
		...
		public void Close(GrantOpening grantOpening)
		{
			...
			if (grantOpening.State == GrantOpeningStates.Unscheduled)
				throw new InvalidOperationException("Cannot close an unscheduled Grant Opening.");
		...
		public IDictionary<int, decimal[]> GetGrantOpeningFinancialsInTrainingPeriods(int fiscalYearId, int programId, int grantStreamId, decimal unclaimedSlippageRate)
		{
			var statesForTotalNumberOfAgreements = StateExtensions.GetInternalStatesForSummary();

			var statesForUnclaimedCommitments = new List<ApplicationStateInternal> {
			...
			var grantOpenings = _dbContext.GrantOpenings
				...
				.Where(x => x.TrainingPeriod.FiscalYearId == fiscalYearId
				            ...
				            && x.GrantStreamId == grantStreamId)
				.ToList();

			var returnVar = new Dictionary<int, decimal[]>();

			foreach (var grantOpening in grantOpenings)
			{
				var claimType = grantOpening?.GrantStream?.GrantProgram?.ProgramConfiguration?.ClaimTypeId;

				if (grantOpening == null)
					continue;

				var results = new decimal[8];

				var paymentRequestTotal = 0m;
				...
				{
					var statesForRequestedPayments = new List<ApplicationStateInternal> {
					...
					paymentRequestTotal = grantOpening.GrantApplications.Where(ga => statesForRequestedPayments.Contains(ga.ApplicationStateInternal))
						.Sum(x => x.Claims.OrderByDescending(cl => cl.Id)
									  .ThenBy(cl => cl.ClaimVersion)
									  .FirstOrDefault()?.TotalAssessedReimbursement ?? 0);

					// Payments Processed: Must sort payment request to get the final one.
					processedPaymentsTotals = grantOpening.GrantApplications.Where(ga => statesForProcessedPayments.Contains(ga.ApplicationStateInternal))
				...
				{
					// Current Payment Requests: Must sum all claim totals where they are in a payment requested state.
					paymentRequestTotal = grantOpening.GrantApplications.Where(ga => statesForTotalNumberOfAgreements.Contains(ga.ApplicationStateInternal))
						.SelectMany(x => x.Claims)
						.Where(c => c.IsPaymentRequested())
						.Sum(c => c.TotalAssessedReimbursement);

					// Payments Processed: must sum all claim totals where they're in an approved state
					processedPaymentsTotals = grantOpening.GrantApplications.Where(ga => statesForTotalNumberOfAgreements.Contains(ga.ApplicationStateInternal))
						.SelectMany(x => x.Claims)
				...
				var unclaimedCommitments = grantOpening.GrantApplications
					.Where(ga => statesForUnclaimedCommitments.Contains(ga.ApplicationStateInternal))
					.Sum(ga => ga.TrainingCost.AgreedCommitment);

				// Projected slippage. Current unclaimed commitments * percentage
				var projectedSlippage = -1 * unclaimedCommitments * unclaimedSlippageRate;
				...
				var slippageToDateAmount = totalScheduleAAmount - paymentRequestTotal;

				// Total Number of Agreements
				results[0] = grantOpening.GrantApplications.Count(ga => statesForTotalNumberOfAgreements.Contains(ga.ApplicationStateInternal));

				// Current Payment Requests
				results[1] = paymentRequestTotal;

				// Payments Processed
				results[2] = processedPaymentsTotals;

				results[3] = projectedSlippage;
				results[4] = totalScheduleAAmount;
				results[5] = slippageToDateAmount;

				// Overpayments, entered manually on Claims Dash
				results[6] = grantOpening.TrainingPeriod.OverpaymentAmount;
				results[7] = unclaimedCommitments;

				returnVar.Add(grantOpening.TrainingPeriodId, results);
			}
