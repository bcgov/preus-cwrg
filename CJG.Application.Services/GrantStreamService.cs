	...
	public class GrantStreamService : Service, IGrantStreamService
	{
		...
		public IEnumerable<ReportRate> GetReportRates(int grantStreamId)
		{
			return _dbContext.ReportRates
				.AsNoTracking()
				.Where(rr => rr.GrantStreamId == grantStreamId);
		}
		...
		{
			return Get<GrantStream>(grantStreamId)
				.ProgramConfiguration.EligibleExpenseTypes
				.Where(t => t.IsActive)
		...
		{
			if (program == null)
				program = GetDefaultGrantProgramId();

			var streamIds = _dbContext.GrantOpenings.AsNoTracking()
				.Where(x => x.TrainingPeriod.FiscalYearId == year)
				.Where(x => x.GrantStream.GrantProgramId == program)
		...
		public IEnumerable<GrantStreamEligibilityQuestion> GetGrantStreamQuestions(int grantStreamId)
		{
			return _dbContext.GrantStreamEligibilityQuestions
				...
				.OrderBy(n => n.RowSequence);
		}

		public IEnumerable<ChecklistCategory> GetGrantStreamChecklist(int grantStreamId)
		{
			return _dbContext.ChecklistCategories
				.Where(c => c.GrantStreamId == grantStreamId)
				.OrderBy(n => n.RowSequence);
		}
		...
		public void UpdateGrantStreamChecklist(IEnumerable<ChecklistCategory> categories, List<int> removeItemIds)
		{
			if (categories == null)
			...
			foreach (var removeItemId in removeItemIds)
			{
				var item = _dbContext.ChecklistItems.FirstOrDefault(i => i.Id == removeItemId);
				if (item != null)
