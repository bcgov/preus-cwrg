...
namespace CJG.Application.Services
{
	public class NationalOccupationalClassificationService : Service, INationalOccupationalClassificationService
	{
		...
		public IEnumerable<NationalOccupationalClassification> GetNationalOccupationalClassificationsInUse()
		{
			var defaultGrantProgramId = GetDefaultGrantProgramId();
			return _dbContext.ProgramDescriptions
				.AsNoTracking()
				.Where(c => c.GrantApplication.ApplicationStateInternal != ApplicationStateInternal.Draft)
				.Where(c => c.GrantApplication.GrantOpening.GrantStream.GrantProgramId == defaultGrantProgramId)
				.Select(c => c.NationalOccupationalClassification)
				.Distinct()
				.OrderBy(x => x.Code)
				.ThenBy(x => x.NOCVersion);
		}
