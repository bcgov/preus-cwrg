...
namespace CJG.Application.Services
{
	...
	public class GrantProgramService : Service, IGrantProgramService
	{
		...
		public IEnumerable<GrantProgram> GetAll(GrantProgramStates? state = null)
		{
			...
			return _dbContext.GrantPrograms
				.Where(gp => gp.Id == defaultProgramId)
				.Where(gp => state == null || gp.State == state)
				.ToArray();
		}
		...
		public IEnumerable<GrantProgram> GetWithActiveStreams()
		{
			...
			return _dbContext.GrantPrograms
				.Where(gp => gp.Id == defaultGrantProgramId)
				.Where(gp => gp.GrantStreams.Any(gs => gs.IsActive) && gp.State == GrantProgramStates.Implemented).ToArray();
		}

		/// <summary>
		/// Get the current users preferred grant programs.
		/// </summary>
		/// <returns></returns>
		public IEnumerable<GrantProgram> GetImplementedGrantPrograms()
		{
			var defaultGrantProgramId = GetDefaultGrantProgramId();
		...
		public void Delete(GrantProgram grantProgram)
		{
			...
			if (grantProgram.AccountCodeId.HasValue)
			{
				...
				var accountCode = _dbContext.AccountCodes.FirstOrDefault(t => t.Id == accountCodeId);
				_dbContext.AccountCodes.Remove(accountCode);
			}

			if (_dbContext.GrantPrograms.Count(t =>
				    t.ApplicantDeclarationTemplateId == grantProgram.ApplicantDeclarationTemplateId
			...
			if (_dbContext.GrantPrograms.Count(t =>
				    t.ApplicantCoverLetterTemplateId == grantProgram.ApplicantCoverLetterTemplateId
				    && t.Id != grantProgram.Id) == 0)
			{
			...
			if (_dbContext.GrantPrograms.Count(t =>
				    t.ApplicantScheduleATemplateId == grantProgram.ApplicantScheduleATemplateId
				    && t.Id != grantProgram.Id) == 0)
			{
			...
			if (_dbContext.GrantPrograms.Count(t =>
				    t.ApplicantScheduleBTemplateId == grantProgram.ApplicantScheduleBTemplateId
				    && t.Id != grantProgram.Id) == 0)
			{
			...
			if (_dbContext.GrantPrograms.Count(t =>
				    t.ParticipantConsentTemplateId == grantProgram.ParticipantConsentTemplateId
				    && t.Id != grantProgram.Id) == 0)
			{
			...
			var notifications = _dbContext.GrantProgramNotificationTypes.Where(t => t.GrantProgramId == grantProgram.Id)
				.ToList();
		...
		public void Terminate(GrantProgram grantProgram)
		{
			...
			if (_dbContext.GrantStreams.Any(t => t.GrantProgramId == grantProgram.Id && t.IsActive))
			{
		...
		public string GenerateApplicantDeclarationBody(GrantApplication grantApplication)
		{
			if (grantApplication.GrantOpening.GrantStream.GrantProgram.ApplicantDeclarationTemplate == null)
				throw new InvalidOperationException($"Cannot find applicant declaration document template '{DocumentTypes.ApplicantDeclaration}'.");
