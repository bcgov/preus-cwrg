			...
			var users = _dbContext.Users
				...
				.Select(x =>
					new KeyValuePair<string, string>(
						x.BCeIDGuid.ToString(),
						$"{x.Organization?.LegalName}{(string.IsNullOrEmpty(x.Organization?.DoingBusinessAs) ? "" : "/" + x.Organization?.DoingBusinessAs)} - {x.LastName}, {x.FirstName}"))
				.ToList();
