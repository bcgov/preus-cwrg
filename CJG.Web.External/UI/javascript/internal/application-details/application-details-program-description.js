app.controller('ProgramDescription', function ($scope, $attrs, $controller, $timeout, Utils) {
  ...
  if (typeof ($scope.applicantOrganizationTypes) === 'undefined')
    $scope.applicantOrganizationTypes = [];

  if (typeof ($scope.participantEmploymentStatuses) === 'undefined')
    $scope.participantEmploymentStatuses = [];

  if (typeof ($scope.underRepresentedPopulations) === 'undefined')
    $scope.underRepresentedPopulations = [];

  if (typeof ($scope.vulnerableGroups) === 'undefined')
    $scope.vulnerableGroups = [];

  if (typeof ($scope.naics1) === 'undefined')
    $scope.naics1 = [];

  if (typeof ($scope.nocs1) === 'undefined')
    $scope.nocs1 = [];

  if (typeof ($scope.communities) === 'undefined')
    $scope.communities = [];
   ...
   **/
  function getSelectedNAICS () {
    if ($scope.model.Naics5Id)
      return getCaption($scope.programNAICS5, $scope.model.Naics5Id);

    if ($scope.model.Naics4Id)
      return getCaption($scope.programNAICS4, $scope.model.Naics4Id);

    if ($scope.model.Naics3Id)
      return getCaption($scope.programNAICS3, $scope.model.Naics3Id);

    if ($scope.model.Naics2Id)
      return getCaption($scope.programNAICS2, $scope.model.Naics2Id);

    if ($scope.model.Naics1Id)
      return getCaption($scope.naics1, $scope.model.Naics1Id);
  }
   ...
   **/
  function getSelectedNOC () {
    if ($scope.model.Noc5Id)
      return getCaption($scope.programNOCs5, $scope.model.Noc5Id);

    if ($scope.model.Noc4Id)
      return getCaption($scope.programNOCs4, $scope.model.Noc4Id);

    if ($scope.model.Noc3Id)
      return getCaption($scope.programNOCs3, $scope.model.Noc3Id);

    if ($scope.model.Noc2Id)
      return getCaption($scope.programNOCs2, $scope.model.Noc2Id);

    if ($scope.model.Noc1Id)
      getCaption($scope.nocs1, $scope.model.Noc1Id);
  }
   ...
   **/
  function getSelectedCommunities() {
    if (!$scope.model.SelectedCommunityIds || !$scope.model.SelectedCommunityIds.length || !$scope.communities.length)
      return;
