app.controller('NotificationTypePreview', function ($scope, $attrs, $controller, $timeout, Utils, ngDialog) {
  ...
  function loadRecipientRules() {
    return $scope.load({
      url: '/Int/Admin/Notification/RecipientRules',
      set: 'recipientRules'
    });
  }
  ...
  function init() {
    ...
    return Promise.all([
      ...
      .then(function() {
        ...
        if ($scope.grantPrograms.length > 0)
          $scope.model.RecipientRule = $scope.recipientRules[0].Key;
  ...
  $scope.loadNotificationTypePreview = function () {
    return $scope.load({
      ...
      condition: $scope.model.GrantProgramId && $scope.model.RecipientRule && $scope.model.ApplicationStateInternal !== null
    });
  };
