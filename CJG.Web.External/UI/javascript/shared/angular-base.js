...
app.controller('Base', function ($scope, $sce, $timeout, Utils, ngDialog) {
  ...
  * @param {any} [options.data] - The data to send in the AJAX request, or a function to retrieve the data.
  * @param {object} [options.headers] - An object containing HTTP headers to include in the request (default includes; RequestVerificationToken, X-Requested-With, Cache-Control, Pragma, If-Modified-Since).
  ...
  */
  ...
  $scope.attachmentDialog = function (title, attachment) {
    ...
    return ngDialog.openConfirm({
      ...
      controller: function ($scope, Utils) {
         ...
         **/
        $scope.save = function () {
          var existingAttachment = $scope.ngDialogData.attachment.Id > 0;
          var haveFileData = $scope.ngDialogData.attachment.File != null && $scope.ngDialogData.attachment.File.name.length > 0;

          var allowSave = existingAttachment || haveFileData;
          if (allowSave) {
  ...
  $scope.evaluationAttachmentDialog = function (title, attachment) {
    ...
    return ngDialog.openConfirm({
      template: '/content/dialogs/_EvaluationAttachments.html',
      data: {
        title: title,
        attachment: attachment
      },
  ...
  $scope.historyAttachmentDialog = function (title, attachment) {
    if (!title) title = 'Documents';
    if (!attachment) attachment = Object.assign({
      Id: 0
    }, attachment);

    return ngDialog.openConfirm({
      template: '/content/dialogs/_HistoryAttachments.html',
      data: {
        title: title,
        attachment: attachment
      },
   ...
   * @returns {Promise}
   */
  $scope.noteAttachmentDialog = function (title, attachment) {
    if (!title) title = 'Attachment';
    if (!attachment) attachment = Object.assign({
      Id: 0
    }, attachment);
   ...
   * @returns {Promise}
   */
  $scope.successStoryAttachmentDialog = function (title, attachment) {
    if (!title)
      title = 'Attachment';
    ...
    return ngDialog.openConfirm({
      template: '/content/dialogs/_SuccessStoryAttachments.html',
      data: {
        title: title,
        attachment: attachment
      },
      controller: function ($scope, Utils) {
        ...
        $scope.save = function () {
          ...
          var allowSave = existingAttachment || haveFileData;
          if (allowSave) {
            let attachmentTypeName = $scope.ngDialogData.attachment.AttachmentType === 11 ? "Testimonial" : "Consent Form";
            $scope.ngDialogData.attachment.AttachmentTypeName = attachmentTypeName;
            return $scope.confirm($scope.ngDialogData.attachment);
          } else {
            Utils.initValue($scope, 'errors.File', 'A file is required.');
          }
