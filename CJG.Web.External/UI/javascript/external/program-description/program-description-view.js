  ...
  $scope.section = {
    ...
    save: {
      ...
      data: function () {
        ...
        $scope.model.SelectedParticipantEmploymentStatusIds = $scope.model.ParticipantEmploymentStatuses.filter(function (item) { return item.Selected; }).map(function (item) { return item.Key; });
        return $scope.model;
      }
    },
    ...
    return $scope.load({
      ...
      .then(function (response) {
        return $timeout(function () {
          ...
          $scope.model.ParticipantEmploymentStatuses.forEach(function (item) {
            item.Selected = $scope.model.SelectedParticipantEmploymentStatusIds.some(function (id) { return item.Key === id; });
          });
    ...
    return $scope.load({
      ...
      .then(function (response) {
        if ($scope.model.costCheck.CostExceeded == true) {
          return $scope.confirmDialog('Project Description Confirmation', '<p>Lowering the number of participants results in exceeding the per participant cost limits.  If you want to proceed with a lower number of participants, your costs will be reset.</p>')
            .then(function () {
              return $scope.save();
            });
        } else {
