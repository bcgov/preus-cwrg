...
namespace CJG.Web.External.Helpers
{
	...
	{
		public static ValidationResult ValidateCommunitiesForApplicant(List<int> communityIds, ValidationContext context)
		{
			if (communityIds?.Count == 0)
				return new ValidationResult("Community Names are required.");

			if (communityIds?.Count > 5)
				return new ValidationResult("Please select a maximum of 5 communities.");
		...
		{
			var model = context.ObjectInstance as Areas.Int.Models.ProgramDescriptions.ProgramDescriptionViewModel;
			var limitCommunities = model?.LimitCommunities ?? true;

			if (communityIds?.Count == 0)
				return new ValidationResult("Community Names are required.");

			if (limitCommunities && communityIds?.Count > 5)
				return new ValidationResult("Please select a maximum of 5 communities.");
		...
		public static ValidationResult ValidateNAICS(int? naicsLevelId, ValidationContext context)
		{
			if (context.ObjectInstance is Areas.Ext.Models.ProgramDescriptions.ProgramDescriptionViewModel extModel)
			{
				...
				{
					case "Naics1Id":
						if (naicsLevelId == null)
							return new ValidationResult("NAICS is required to three levels.");
						break;

					case "Naics2Id":
						if (naicsLevelId == null && extModel.Naics1Id != null)
							return new ValidationResult("NAICS is required to three levels.");
						break;

					case "Naics3Id":
						if (naicsLevelId == null && extModel.Naics2Id != null)
							return new ValidationResult("NAICS is required to three levels.");
						break;
				}
			}
			else if (context.ObjectInstance is Areas.Int.Models.ProgramDescriptions.ProgramDescriptionViewModel intModel)
			{
				switch (context.MemberName)
				{
					...
					case "Naics2Id":
						if (naicsLevelId == null && intModel.Naics1Id != null)
							return new ValidationResult("NAICS is required to three levels.");
						break;

					case "Naics3Id":
						if (naicsLevelId == null && intModel.Naics2Id != null)
							return new ValidationResult("NAICS is required to three levels.");
						break;
				}
			}
		...
		public static ValidationResult ValidateCIPS(int? CipsCodeId, ValidationContext context)
		{
			if (context.ObjectInstance is Areas.Int.Models.TrainingPrograms.TrainingProgramViewModel ETGTrainingProgramModel)
			{
				switch (context.MemberName)
				{
					case "CipsCode1Id":
						if (CipsCodeId == null)
							return new ValidationResult("CIPS is required to three levels.");
						break;

					case "CipsCode2Id":
						if (CipsCodeId == null && ETGTrainingProgramModel.CipsCode1Id != null)
							return new ValidationResult("CIPS is required to three levels.");
						break;

					case "CipsCode3Id":
						if (CipsCodeId == null && ETGTrainingProgramModel.CipsCode2Id != null)
							return new ValidationResult("CIPS is required to three levels.");
						break;
				}
			}
			else if (context.ObjectInstance is Areas.Int.Models.SkillsTraining.SkillsTrainingProgramViewModel WDASkillTrainingProgramModel)
			{
				switch (context.MemberName)
				{
					...
					case "CipsCode2Id":
						if (CipsCodeId == null && WDASkillTrainingProgramModel.CipsCode1Id != null)
							return new ValidationResult("CIPS is required to three levels.");
						break;

					case "CipsCode3Id":
						if (CipsCodeId == null && WDASkillTrainingProgramModel.CipsCode2Id != null)
							return new ValidationResult("CIPS is required to three levels.");
						break;
				}
			}
