...
namespace CJG.Web.External.Helpers
{
    public class ClaimDashboardViewModelBuilder
	{
		...
		public ClaimDashboardViewModelBuilder(IStaticDataService staticDataService, IGrantOpeningService grantOpeningService, IGrantProgramService grantProgramService, IGrantStreamService grantStreamService, IReportRateService reportRateService, IFiscalYearService fiscalYearService, IPrincipal user)
		{
		...
		public ClaimDashboardViewModelBuilder Build(ClaimDashboardViewModel model)
		{
			...
			if (!_model.SelectedGrantProgramId.HasValue)
				_model.SelectedGrantProgramId = _grantProgramService.GetDefaultGrantProgramId();

			FiscalYear fiscalYear;
			if (_model.SelectedFiscalYearId.HasValue)
			{
				fiscalYear = _fiscalYearService.Get(_model.SelectedFiscalYearId.Value);
			}
			else
			{
				fiscalYear = _fiscalYearService.GetCurrentFiscalYear();
			...
			var grantOpeningFinancialArray = _grantOpeningService.GetGrantOpeningFinancialsInTrainingPeriods(fiscalYear.Id, program.Id, stream.Id, (decimal)_model.UnclaimedSlippageRate);
			...
			{
				var grantOpeningExists = grantOpeningFinancialArray.ContainsKey(trainingPeriod.Id);
				DefaultValueForTrainingPeriod(grantOpeningFinancialArray, trainingPeriod, new decimal[] { 0, 0, 0, 0, 0, 0, 0, 0 });
				DefaultValueForTrainingPeriod(budgetAllocationAmounts, trainingPeriod);

				var grantOpeningFinanicalsForPeriod = new GrantOpeningFinanicals(grantOpeningFinancialArray[trainingPeriod.Id]);
