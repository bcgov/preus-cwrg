...
namespace CJG.Web.External.Helpers
{
	public static class SidebarViewModelFactory
	{
		...
		{
			return Create(grantApplication, GetPath(controllerContext), GetBaseUrl(controllerContext));
		}

		public static SidebarViewModel Create(GrantApplication grantApplication, string path, string baseUrl)
		{
			return SidebarBuilder.Build(grantApplication, path, baseUrl);
		}
		...
		private static string GetBaseUrl(ControllerContext controllerContext)
		{
			if (controllerContext.HttpContext?.Request != null && controllerContext.HttpContext.Request.Url != null)
				return controllerContext.HttpContext.Request.Url?.GetLeftPart(UriPartial.Authority);
			return string.Empty;
		}
	}
	...
	public class SidebarBuilder
	{
		...
		public SidebarViewModel Build(GrantApplication grantApplication, string path, string baseUrl)
		{
			var sidebarViewModel = new SidebarViewModel(grantApplication,
				_links.Where(x =>
			...
			{
				var exitFormUrl = $"{baseUrl}/Part/ExitForm/{grantApplication.InvitationKey}";
				sidebarViewModel.SideBarLinks.Add(SidebarLinkType.ExitForm, new SidebarLinkViewModel("Exit Form", exitFormUrl));
			}
