    ...
    public abstract class BaseController : Controller
	{
		...
		protected void HandleModelStateValidation(BaseViewModel model, string summary = null)
		{
			if (model.ValidationErrors == null)
				...
				model.ValidationErrors.AddRange(GetClientErrors());
		...
		{
			string message;
			var validationError = error as DbEntityValidationException;

			var type = error.GetType();
			if (type == typeof(DbEntityValidationException))
			{
				message = validationError.GetValidationMessages(delimiter);
			}
