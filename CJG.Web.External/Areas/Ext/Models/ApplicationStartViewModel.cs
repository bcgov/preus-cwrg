...
namespace CJG.Web.External.Areas.Ext.Models
{
	public class ApplicationStartViewModel : BaseViewModel
	{
			...
			: this(grantApplication.GrantOpening.GrantStream.GrantProgramId, grantOpeningService, grantProgramService, staticDataService, grantStreamService)
		{
			...
			{
				foreach (var questionModel in GrantStream.StreamEligibilityQuestions)
				{
					if (answer.GrantStreamEligibilityQuestionId == questionModel.Id)
					{
						questionModel.EligibilityAnswer = answer.EligibilityAnswer;
						questionModel.Designation = (int?)answer.GrantWriterDesignation;
		...
		public ApplicationStartViewModel(int grantProgramId, IGrantOpeningService grantOpeningService, IGrantProgramService grantProgramService, IStaticDataService staticDataService, IGrantStreamService grantStreamService)
		{
			...
			if (grantProgramService == null)
				throw new ArgumentNullException(nameof(grantProgramService));
			...
			var grantOpenings = grantOpeningService
				.GetGrantOpenings(AppDateTime.UtcNow, GrantProgramId)
				.GroupBy(o => new { o.TrainingPeriod.StartDate, o.TrainingPeriod.EndDate });
			...
			TrainingPeriods = grantOpenings.Select(
				o => new ApplicationGrantOpeningGroupModel(
					...
					o.Key.EndDate,
					ProgramType,
					o.Select(g => new ApplicationGrantOpeningViewModel(g)).ToList()))
				.ToList();
