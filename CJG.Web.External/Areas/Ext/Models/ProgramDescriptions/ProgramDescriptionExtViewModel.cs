...
namespace CJG.Web.External.Areas.Ext.Models.ProgramDescriptions
{
	...
	public class ProgramDescriptionExtViewModel : ProgramDescriptionViewModel
	{
		...
		{
			ParticipantEmploymentStatuses = _staticDataService.GetParticipantEmploymentStatuses().Select(x => new KeyValuePair<int, string>(x.Id, x.Caption)).ToArray();
			ApplicantOrganizationTypes = _staticDataService.GetApplicantOrganizationTypes().Select(x => new KeyValuePair<int, string>(x.Id, x.Caption)).ToArray();
			Communities = _communityService.GetAll().Where(t => t.IsActive).Select(x => new KeyValuePair<int, string>(x.Id, x.Caption)).ToArray();
			VulnerableGroups = _staticDataService.GetVulnerableGroups().Select(x => new KeyValuePair<int, string>(x.Id, x.Caption)).ToArray();
			UnderRepresentedPopulations = _staticDataService.GetUnderRepresentedPopulations().Select(x => new KeyValuePair<int, string>(x.Id, x.Caption)).ToArray();
