...
namespace CJG.Web.External.Areas.Ext.Controllers
{
	...
	public class GrantAgreementController : BaseController
	{
		...
		/// <summary>
		/// Get the agreement data for the review agrement view.
		/// </summary>
		/// <param name="grantApplicationId"></param>
		/// <returns></returns>
		[HttpGet]
				...
				switch (document)
				{
					case ("CoverLetter"):
						if (!model.Confirmation)
							AddAngularError(model, "Confirmation", "You must confirm the cover letter.");
		...
		public JsonResult AcceptAgreement(Models.Agreements.AgreementReviewViewModel model)
		{
			try
			{
				if (!model.CoverLetterConfirmed)
					ModelState.AddModelError("CoverLetterConfirmed", "You must accept the cover letter before accepting the agreement.");
