...
namespace CJG.Web.External.Areas.Ext.Controllers
{
	...
	public class AlternateContactController : BaseController
	{
		...
		public JsonResult GetApplicantsForOrganization(int grantApplicationId)
		{
			...
			{
				var grantApplication = _grantApplicationService.Get(grantApplicationId);
				model = _grantApplicationService.GetAvailableApplicationContacts(grantApplication)
					.Select(a => new KeyValuePair<int, string>(a.Id, $"{a.GetUserFullName()} | {a.JobTitle} | {a.EmailAddress} | {a.PhoneNumber}"))
					.ToList();
			}
			catch (Exception ex)
			{
