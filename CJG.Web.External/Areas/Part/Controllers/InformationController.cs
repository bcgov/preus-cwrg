	...
	public class InformationController : BaseController
	{
		...
		public ActionResult Step2(ParticipantInfoViewModel model)
		{
			try
			{
				...
				{
					var maxParticipantEnrolled = IsMaxParticipantEnrolled(model.ParticipantInfoStep0ViewModel.InvitationKey);
					if (maxParticipantEnrolled != null)
						return maxParticipantEnrolled;
				}
		...
		public ActionResult DownloadAttachment()
		{
			...
			{
				this.SetAlert($"The sample {filename} could not be found.", AlertType.Warning, true);
		...
		public ActionResult AddAttachment(HttpPostedFileBase[] files, string component)
		{
			...
			if (files != null)
			{
				...
				foreach (var file in files)
				{
					try
					{
						var attachmentType = new AttachmentModel();
		...
		private void PrepareStep1(ref ParticipantInfoViewModel model, Guid invitationKey)
		{
			...
			model.ParticipantInfoStep1ViewModel = new ParticipantInfoStep1ViewModel
			{
				...
				ProgramStartDate = grantApplication.GetTrainingStartDate().ToLocalMorning(),
