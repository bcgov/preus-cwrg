...
namespace CJG.Web.External.Areas.Int.Controllers
{
	...
	public class GrantStreamController : BaseController
	{
		...
		public JsonResult GetGrantStreams(int grantProgramId)
		{
			...
			try
			{
				list = _grantStreamService.GetGrantStreamsForProgram(grantProgramId, false)
					.Select(t => new
					{
						t.Id,
						Caption = t.Name,
		...
		public JsonResult UpdateGrantStreamQuestions(GrantStreamViewModel fullmodel)
		{
				...
				else
				{
					...
					for (int aIdx = 0; aIdx < model.Count; aIdx++)
					{
						...
						if (clientQuestion.Id != 0)
						{
							...
							dbQuestion.EligibilityRequirements = clientQuestion.EligibilityRequirements ?? string.Empty;
							...
							dbQuestion.CollectContactInformation = clientQuestion.CollectContactInformation;
							dbQuestion.IsActive = clientQuestion.IsActive;
							dbQuestion.RowVersion = Convert.FromBase64String(clientQuestion.RowVersion);
						}
						else
						{
							var newQuestion = new GrantStreamEligibilityQuestion
							{
								EligibilityRequirements = clientQuestion.EligibilityRequirements ?? string.Empty,
								EligibilityQuestion = clientQuestion.EligibilityQuestion,
								EligibilityPositiveAnswerRequired = clientQuestion.EligibilityPositiveAnswerRequired,
								CollectContactInformation = clientQuestion.CollectContactInformation,
								IsActive = clientQuestion.IsActive,
								RowSequence = clientQuestion.RowSequence,
								GrantStreamId = clientQuestion.GrantStreamId
							};
            ...
            // Fix up without the [] since that is not easily addressable inside javascript.
		...
		[ValidateRequestHeader]
		[PreventSpam]
		[Route("Streams/UpdateChecklist")]
		public JsonResult UpdateChecklist(GrantStreamViewModel model)
		{
			int grantStreamId = model.Id;
			var checklistModel = model.CheckListCategories;
			...
			try
			{
				...
				{
					var grantStream = _grantStreamService.Get(model.Id);
					var list = _grantStreamService.GetGrantStreamChecklist(grantStreamId).ToList();
					var removeItemIds = new List<int>();

					foreach (var category in checklistModel)
					{
						ChecklistCategory checkListCategory;
						if (category.Id != 0)
						{
							checkListCategory = list.First(x => x.Id == category.Id);

							checkListCategory.Caption = category.Caption ?? string.Empty;
							checkListCategory.IsActive = category.IsActive;
							checkListCategory.RowSequence = category.RowSequence;
							checkListCategory.RowVersion = Convert.FromBase64String(category.RowVersion);
						}
						else
						{
							checkListCategory = new ChecklistCategory();
							checkListCategory.Caption = category.Caption;
							checkListCategory.IsActive = category.IsActive;
							checkListCategory.RowSequence = category.RowSequence;
							checkListCategory.GrantStreamId = category.GrantStreamId;

							list.Add(checkListCategory);
						}

						var postedItemIds = category.Items
							.Where(i => i.Id > 0)
							.Select(i => i.Id)
							.ToList();

						var itemsToRemove = checkListCategory.Items
							.Where(i => !postedItemIds.Contains(i.Id))
							.Select(i => i.Id)
							.ToList();

						removeItemIds.AddRange(itemsToRemove);
						...
						foreach (var item in category.Items)
						{
							...
							if (item.Id == 0)
							{
								...
								checkListCategory.Items.Add(checklistItem);
							}

							if (item.Id > 0)
							{
								checklistItem = checkListCategory.Items.First(x => x.Id == item.Id);
								checklistItem.RowVersion = Convert.FromBase64String(item.RowVersion);
							}

							checklistItem.Caption = item.Caption;
							checklistItem.RowSequence = item.RowSequence;
							checklistItem.IsActive = item.IsActive;
						}
					}

					_grantStreamService.UpdateGrantStreamChecklist(list, removeItemIds);
					model = new GrantStreamViewModel(grantStream, _serviceCategoryService, _staticDataService, _grantStreamService);
				}
			}
			catch (Exception ex)
