	...
	public class ApplicationController : BaseController
	{
		...
		public JsonResult GetApplicationDetails(int grantApplicationId)
		{
			...
			try
			{
				...
				var evaluationFormQuestions = _evaluationFormService
					.GetQuestions()
					.ToList();

				model.AllEvaluationQuestionsAnswered = grantApplication.AllEvaluationItemsAnswered(evaluationFormQuestions);
				model.AllowEvaluationPrint = grantApplication.EvaluationCanBePrinted();
				model.AllowEvaluationWithdrawal = grantApplication.EvaluationCanBeWithdrawn();
		...
		public JsonResult GetAssessors(int grantApplicationId)
		{
			...
			try
			{
				_grantApplicationService.Get(grantApplicationId);  // Cheap access test
		...
		[HttpGet]
		[PreventSpam]
		[Route("Application/Attachment/View/{grantApplicationId}/{attachmentId}")]
		public ActionResult ViewAttachment(int grantApplicationId, int attachmentId)
		{
			...
			try
			{
				...
				var mimeType = MimeMapping.GetMimeMapping(attachment.FileName);
				Response.AddHeader("Content-Disposition", $"inline; filename={attachment.FileName}");

				return File(attachment.AttachmentData, mimeType);
			}
			catch (Exception ex)
			{
				HandleAngularException(ex, model);
