...
namespace CJG.Web.External.Areas.Int.Controllers
{
	...
	public class ClaimIntakeController : BaseController
	{
		...
		public JsonResult GetClaimManagementDashboard()
		{
			...
			try
			{
				model = new ClaimDashboardViewModelBuilder(_staticDataService, _grantOpeningService, _grantProgramService, _grantStreamService, _reportRateService, _fiscalYearService, User).Build(model).Calculate();
			}
			catch (Exception ex)
			{
		...
		public JsonResult RefreshClaimManagementDashboard(ClaimDashboardViewModel viewModel)
		{
			...
			try
			{
				viewModel.UnclaimedSlippageRate = 0;
				viewModel.UnclaimedCancellationRate = 0;
				viewModel.ClaimedSlippageRate = 0;

				model = new ClaimDashboardViewModelBuilder(_staticDataService, _grantOpeningService, _grantProgramService, _grantStreamService, _reportRateService, _fiscalYearService, User)
					.Build(viewModel)
					.Calculate();
			}
			catch (Exception ex)
		...
		public JsonResult SaveClaimManagementDashboard(ClaimDashboardViewModel viewModel)
		{
			...
			try
			{
				...
				model = new ClaimDashboardViewModelBuilder(_staticDataService, _grantOpeningService, _grantProgramService, _grantStreamService, _reportRateService, _fiscalYearService, User).Build(viewModel).Calculate();
			}
			catch (Exception ex)
			{
				HandleAngularException(ex, model);
