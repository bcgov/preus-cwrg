...
namespace CJG.Web.External.Areas.Int.Models.Claims
{
    public class ClaimAssessmentViewModel : BaseViewModel
	{
		...
		public ClaimAssessmentViewModel(Claim claim, IPrincipal user, Func<string, string> GetWorkflowUrl)
		{
			...
			EvaluationIsComplete = claim.IsEvaluationComplete();
			...
			switch (ProgramType)
			{
				case ProgramTypes.EmployerGrant:
					...
					Participants = claim.GrantApplication.ParticipantForms
						.Select(p => new ParticipantViewModel(p))
						.ToArray();
					break;

				case ProgramTypes.WDAService:
					Participants = claim.ParticipantForms
						.Select(pf => new ParticipantViewModel(pf))
						.ToArray();
					break;
			}

			AmountPaidOrOwing = claim.ClaimTypeId == ClaimTypes.SingleAmendableClaim
				? claim.AmountPaidOrOwing()
				: claim.GrantApplication.AmountPaidOrOwing();
