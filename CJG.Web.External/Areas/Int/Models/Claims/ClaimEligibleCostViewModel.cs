...
namespace CJG.Web.External.Areas.Int.Models.Claims
{
	public class ClaimEligibleCostViewModel
	{
		...
		public decimal PreviousAssessedParticipantReimbursement { get; set; }
		#endregion
		...
		public decimal AccountsReceivableOverpayment { get; set; }
		...
		public ClaimEligibleCostViewModel(ClaimEligibleCost claimEligibleCost)
		{
				...
				|| claimEligibleCost.AssessedMaxParticipantReimbursementCost != claimEligibleCost.CalculateAssessedMaxParticipantReimbursement())
			{
				if (claimEligibleCost.Claim.GrantApplication.GetProgramType() != ProgramTypes.EmployerGrant)
					OverrideRate = (double)claimEligibleCost.AssessedReimbursementCost / (double)claimEligibleCost.AssessedCost;
			}
			...
			ClaimEmployerContribution = claimEligibleCost.ClaimCost - claimEligibleCost.ClaimReimbursementCost;
			...
			PreviousAssessedParticipantReimbursement = claimEligibleCost.Source?.ParticipantCosts.Sum(pc => pc.AssessedReimbursement) ?? 0;
			...
			AccountsReceivableOverpayment = claimEligibleCost.AccountsReceivableOverpayment;
			AccountsReceivablePaidDate = claimEligibleCost.AccountsReceivablePaidDate;
			AccountsReceivablePaidDateFormatted = claimEligibleCost.AccountsReceivablePaidDate.HasValue ? claimEligibleCost.AccountsReceivablePaidDate.Value.ToString("yyyy-MM-dd") : "--";
		}
	}
}