...
{
	public class ProgramConfigurationViewModel : BaseViewModel
	{
		#region Properties
		...
		#region Constructors
		public ProgramConfigurationViewModel() { }

		public ProgramConfigurationViewModel(ProgramConfiguration programConfiguration)
		{
		...
		#region Methods
		/// <summary>
		/// Copy the values from this model into the specified grant program configuration.
		/// </summary>
		/// <param name="grantProgram"></param>
		/// <param name="_serviceCategoryService"></param>
		/// <param name="_serviceLineService"></param>
		public void MapTo(GrantProgram grantProgram, IServiceCategoryService serviceCategoryService, IServiceLineService serviceLineService)
		{
			if (grantProgram == null) throw new ArgumentNullException(nameof(grantProgram));

			var programConfiguration = grantProgram.ProgramConfiguration ?? new ProgramConfiguration()
			{
				Caption = $"{grantProgram.Name} - {Guid.NewGuid().ToString()}",
				IsActive = true,
			...
			foreach (var expenseType in this.EligibleExpenseTypes)
			{
				...
				else
				{
					...
					foreach (var breakdown in expenseType.Breakdowns)
					{
						...
						else
						{
							...
							eligibleExpenseBreakdown.EnableCustomTitle = serviceLine.EnableCustomTitle;
						}
					}
				}
			}
