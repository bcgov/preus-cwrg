...
namespace CJG.Web.External.Areas.Int.Models.ProgramDescriptions
{
    public class ProgramDescriptionViewModel : BaseViewModel
	{
		...
		public string ApplicationOrganizationTypeWhenInactivated { get; set; }
		...
		public bool LimitCommunities { get; set; } = true;
			...
			bool limitCommunities = true)
		{
			...
			LimitCommunities = limitCommunities;

			if (!programDescription.ApplicantOrganizationType.IsActive)
				ApplicationOrganizationTypeWhenInactivated = $"{programDescription.ApplicantOrganizationType.Caption} (Inactive)";
		...
		{
			if (programDescription == null)
				throw new ArgumentException(nameof(programDescription));

			if (vulnerableGroupService == null)
				throw new ArgumentNullException(nameof(vulnerableGroupService));

			if (underRepresentedPopulationService == null)
				throw new ArgumentNullException(nameof(underRepresentedPopulationService));
