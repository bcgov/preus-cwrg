...
namespace CJG.Testing.UnitTests.Controllers.Int
{
	...
	public class DirectorDashboardControllerTest
	{
		...
		public void GetTrainingPeriods()
		{
			...
			helper.GetMock<IFiscalYearService>().Setup(m => m.GetTrainingPeriodLabels(It.IsAny<int>(), It.IsAny<int?>()))
				.Returns(new List<KeyValuePair<string, string>> { new KeyValuePair<string, string>(trainingPeriod.Caption, trainingPeriod.Caption) });

			// Act
			var result = controller.GetTrainingPeriods(trainingPeriod.FiscalYear.Id, null);

			// Assert
			result.Should().NotBeNull().And.BeOfType<JsonResult>();
			result.Data.Should().NotBeNull();

			var trainingPeriods = result.GetJsonValue<List<KeyValuePair<string, string>>>("TrainingPeriods");

			trainingPeriods.Count.Should().Be(1);
			trainingPeriods.First().Key.Should().Be(trainingPeriod.Caption);
			trainingPeriods.First().Value.Should().Be(trainingPeriod.Caption);
			helper.GetMock<IFiscalYearService>().Verify(m => m.GetTrainingPeriodLabels(It.IsAny<int>(), It.IsAny<int?>()), Times.Once);
		}
		...
		public void GetTrainingPeriods_Exception()
		{
			...
			helper.GetMock<IFiscalYearService>().Setup(m => m.GetTrainingPeriodLabels(It.IsAny<int>(), It.IsAny<int?>())).Throws<NoContentException>();

			var controller = helper.Create();

			// Act
			var result = controller.GetTrainingPeriods(1, 1);
		...
		public void GetGrantApplications()
		{
			...
			helper.GetMock<IGrantApplicationService>()
				...
				.Returns(pageList);
			var filter = new BatchApprovalFilterViewModel
			{
				AssessorId = 1,
				FiscalYearId = 1,
				GrantProgramId = 1,
				GrantStreamId = 1,
				TrainingPeriodCaption = "Intake Period 1"
			};
			...
			// Assert
			result.Should().NotBeNull().And.BeOfType<JsonResult>();
			result.Data.Should().NotBeNull().And.BeOfType<PageList<GrantApplicationViewModel>>();
			var data = result.Data as PageList<GrantApplicationViewModel>;
		...
		public void IssueOffers()
		{
			...
			var batchApprovalViewModel = new BatchApprovalViewModel
			{
				SelectAll = true,
				AssessorId = 1,
				FiscalYearId = 1,
			...
			// Assert
			result.Should().NotBeNull().And.BeOfType<JsonResult>();
			result.Data.Should().NotBeNull().And.BeOfType<BatchApprovalViewModel>();
			var data = result.Data as BatchApprovalViewModel;
