...
namespace CJG.Testing.UnitTests.Controllers.Ext
{
	...
	{
		#region Index
		[Ignore, TestMethod, TestCategory("Controller"), TestCategory(nameof(HomeController))]
		public void Index()
		{
			// Arrange
		...
		public void GetGrantPrograms()
		{
			...
			mockGrantProgramService.Setup(m => m.GetImplementedGrantPrograms()).Returns(new List<GrantProgram> { grantApplication.GrantOpening.GrantStream.GrantProgram });
			...
			helper.GetMock<IGrantProgramService>().Verify(m => m.GetImplementedGrantPrograms(), Times.Once);
		}
		...
		public void GetGrantPrograms_Exception()
		{
			...
			helper.GetMock<IGrantProgramService>().Setup(m => m.GetImplementedGrantPrograms()).Throws<NoContentException>();
