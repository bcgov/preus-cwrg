...
namespace CJG.Testing.UnitTests.Controllers.Ext
{
	...
	public class ApplicationReviewControllerTest
	{
		...
		[Ignore, TestMethod, TestCategory("Controller"), TestCategory(nameof(ApplicationReviewController))]
		...
		public void SubmitApplication()
		{
			...
			helper.GetMock<IGrantApplicationService>().Setup(m => m.Get(It.IsAny<int>())).Returns(grantApplication);
			helper.GetMock<IGrantProgramService>().Setup(m => m.GenerateApplicantDeclarationBody(It.IsAny<GrantApplication>())).Returns(applicantDeclaration.Body);
			helper.GetMock<IOrganizationService>().Setup(m => m.ProfileSubjectToVerification(It.IsAny<Organization>())).Returns(false);
			...
			var model = result.Data as ApplicantDeclarationViewModel;
			model.Id.Should().Be(grantApplication.Id);
			model.RedirectURL.Should().Be("/Ext/Home/Index");
			controller.TempData["Message"].Should().Be("Your application has been submitted.");
		...
		[TestMethod, TestCategory("Controller"), TestCategory(nameof(ApplicationReviewController))]
		public void SubmitApplication_WithOutdatedProfile()
		{
			// Arrange
			var user = EntityHelper.CreateExternalUser();
			var helper = new ControllerHelper<ApplicationReviewController>(user);
			...
			helper.GetMock<IGrantApplicationService>().Setup(m => m.Get(It.IsAny<int>())).Returns(grantApplication);
			helper.GetMock<IGrantProgramService>().Setup(m => m.GenerateApplicantDeclarationBody(It.IsAny<GrantApplication>())).Returns(applicantDeclaration.Body);
			helper.GetMock<IOrganizationService>().Setup(m => m.ProfileSubjectToVerification(It.IsAny<Organization>())).Returns(true);
			...
			var model = (ApplicantDeclarationViewModel)result.Data;
			model.Id.Should().Be(grantApplication.Id);
			model.RedirectURL.Should().Be("/OrganizationProfile/OrganizationProfileView");  // In IIS, will be /Ext/Organization/Profile/View due to routing being the real thing
			controller.TempData["Message"].Should().Be("Your application has been submitted. It has been over 12 months since your Organization Profile was updated. Please verify that your profile is correct, and hit 'Save Organization Profile'.");

			helper.GetMock<IOrganizationService>().Verify(m => m.ProfileSubjectToVerification(It.IsAny<Organization>()), Times.Once);
		}
