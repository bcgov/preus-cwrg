...
{
	internal class DuplicateSinReportJob : IDuplicateSinReportJob
	{
		private readonly ILogger _logger;
		private readonly IParticipantService _participantService;
		private readonly IFiscalYearService _fiscalYearService;

		public DuplicateSinReportJob(IParticipantService participantService, IFiscalYearService fiscalYearService, ILogger logger)
		{
		...
		public void Start(DateTime currentDate, string csvFilePath, string htmlFilePathTemplate, int daysBefore, string templatePath, DateTime cutoffDate, int maxParticipants = 1000, bool addHeader = false)
		{
			var currentFiscalYear = _fiscalYearService.GetCurrentFiscalYear();

			_logger.Info($"Duplicate SIN: Retrieving participants... [current date: {currentDate.ToStringLocalTime()}] [daysBefore: {daysBefore}] [maxParticipants: {maxParticipants}]");

			var participants = _participantService.GetParticipantEnrollmentsWithDuplicatedSIN(currentDate.AddDays(-daysBefore), maxParticipants, cutoffDate, currentFiscalYear.StartDate, currentFiscalYear.EndDate)
				.ToList();

			_logger.Info("Converting and formatting data...");

			var header = addHeader ? CreateCsvHeaderString() : null;
			var placeholders = participants.Select(s => s).Select(ConvertParticipantFormToPlaceholders).ToList();
			...
			_logger.Info("Exporting data into html files...");
			...
			_participantService.UpdateDuplicateSINReportedDate(participants.Select(s => s.PrimaryForm), currentDate);
		...
		private string CreateCsvHeaderString()
		{
			return new CsvLineBuilder()
				...
				.AppendColumn("SIN")
				.AppendColumn("Application Numbers with Duplicated SIN")
		...
		private static IDictionary<string, string> ConvertParticipantFormToPlaceholders(DuplicateSINParticipants duplicatedParticipant)
		{
			var participant = duplicatedParticipant.PrimaryForm;

			var ga = participant.GrantApplication;
			var tp = ga.TrainingPrograms.FirstOrDefault();

			var totalHours = ga.TrainingPrograms.Sum(t => t.TotalTrainingHours);
			var trainingStartDate = ga.TrainingPrograms.Min(programs => programs.StartDate);
			var trainingEndDate = ga.TrainingPrograms.Max(programs => programs.EndDate);

			var duplicatedFilesNumbers = duplicatedParticipant.DuplicatedParticipantForms
			...
			var placeholders = new Dictionary<string, string>
			{
				...
				{"Stream", ga.GrantOpening.GrantStream.Name},
				...
				{"DuplicateFileNumbers", string.Join(", ", duplicatedFilesNumbers)},
				...
				{"TrainingStartDate", trainingStartDate.ToLocalTime().ToString("yyyy-MM-dd")},
				{"TrainingEndDate", trainingEndDate.ToLocalTime().ToString("yyyy-MM-dd")},
				{"HoursPerWeek", GetTrainingHoursPerWeek(totalHours, trainingStartDate, trainingEndDate).ToString("F1")},
