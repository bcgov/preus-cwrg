...
{
	internal class WithdrawalSurveyReportJob : IWithdrawalSurveyReportJob
	{
		private readonly ILogger _logger;
		private readonly IParticipantService _participantService;
		private readonly ISurveyService _surveyService;

		public WithdrawalSurveyReportJob(IParticipantService participantService, ISurveyService surveyService, ILogger logger)
		{
		...
		{
			_logger.Info($"Early Withdrawal Survey: Retrieving participants... [current date: {currentDate.ToStringLocalTime()}] [daysBefore: {daysBefore}] [maxParticipants: {maxParticipants}]");

			var surveyData = _surveyService.GetParticipantsWithWithdrawalSurveyAnswersToReport(currentDate.AddDays(-daysBefore), maxParticipants, cutoffDate);

			_logger.Info("Converting and formatting data...");

			var header = addHeader ? CreateCsvHeaderString(surveyData.Questions) : null;
			var placeholders = surveyData.Participants
				.Select(p => ConvertParticipantFormToPlaceholders(surveyData.Questions, p))
				.ToList();
			...
			_participantService.UpdateWithdrawalSurveyReportedDate(surveyData.Participants, currentDate);

			_logger.Info($"All {surveyData.Participants.Count} exported participant records marked with date {currentDate.ToUniversalTime()}");
		}
		...
		private string CreateCsvHeaderString(List<KeyValuePair<int, string>> questions)
		{
			var headerBuilder = new CsvLineBuilder()
				...
				.AppendColumn("Date of Withdrawal");
		...
		private static IDictionary<string, string> ConvertParticipantFormToPlaceholders(List<KeyValuePair<int, string>> questions, SurveyParticipantModel participantRecord)
		{
			var placeholders = new Dictionary<string, string>
			{
				{ "AgreementNumber", participantRecord.AgreementNumber },
				{ "AgreementHolder", participantRecord.AgreementHolder },
				{ "FirstName", participantRecord.FirstName },
				{ "LastName", participantRecord.LastName },
				{ "Email", participantRecord.Email },
				{ "TrainingStart", participantRecord.TrainingStartDate.ToDateOnly() },
				{ "TrainingEnd", participantRecord.TrainingEndDate.ToDateOnly() },
				{ "DateOfWithdrawal", participantRecord.WithdrawalDate.ToDateOnly() },
			};

			foreach (var question in questions)
			{
				var answerList = participantRecord.Answers.FirstOrDefault(a => a.Key == question.Key);
