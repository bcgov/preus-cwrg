...
{
	internal class SinReportJob : ISinReportJob
	{
		private readonly ILogger _logger;
		private readonly IParticipantService _participantService;

		public SinReportJob(IParticipantService participantService, ILogger logger)
		{
		...
		{
			_logger.Debug($"SIN Report Job: Retrieving participants... [current date: {currentDate.ToStringLocalTime()}] [daysBefore: {daysBefore}] [maxParticipants: {maxParticipants}]");

			//get participants that have a SIN that starts with 9
			var participants = _participantService.GetTemporaryResidentParticipantEnrollments(currentDate.AddDays(-daysBefore), maxParticipants, cutoffDate)
				.ToList();
			...
			_participantService.UpdateSINReportedDate(participants, currentDate);
