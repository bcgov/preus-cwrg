...
namespace CJG.Infrastructure.ReportingService
{
	internal class SdsiReportJob : ISdsiReportJob
	{
		...
		{
			_logger.Debug($"Section 25 Report Job: Retrieving participants... [current date: {currentDate.ToStringLocalTime()}] [daysBefore: {daysBefore}] [maxParticipants: {maxParticipants}]");

			var participants = _participantService.GetUnemployedParticipantEnrollments(currentDate.AddDays(-daysBefore), maxParticipants, cutoffDate)
				.ToList();
