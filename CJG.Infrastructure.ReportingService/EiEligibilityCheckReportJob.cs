...
namespace CJG.Infrastructure.ReportingService
{
	internal class EiEligibilityCheckReportJob : IEiEligibilityCheckReportJob
	{
		...
		public void Start(DateTime currentDate, string csvFilePath, DateTime cutoffDate, int maxParticipants = 1000)
		{
			...
			_logger.Debug($"CWRG EI Eligibility Check Report Job: Retrieving participants... [current date: {currentDate.ToStringLocalTime()}] [maxParticipants: {maxParticipants}]");
		...
		private string CreateCsvHeaderString()
		{
			return new CsvLineBuilder()
				...
				.AppendColumn("Tariff Impacted")
				.AppendColumn("SIN")
				.AppendColumn("NAICS (Previous Employment)")
				.AppendColumn("NOC (Previous Employment)")
				.ToString();
		}
		...
		private static IDictionary<string, string> ConvertParticipantFormToPlaceholders(ParticipantForm participant)
		{
			...
			var placeholders = new Dictionary<string, string>
			{
				...
				{"TariffImpacted", participant.AffectedByTariffs.HasValue ? participant.AffectedByTariffs.Value ? "Yes" : "No" : string.Empty},
				{"SocialInsuranceNumber", participant.SIN},
				{"PreviousNAICS", participant.PreviousEmploymentNaics != null ? participant.PreviousEmploymentNaics.ToString() : string.Empty},
				{"PreviousNOC", participant.PreviousEmploymentNoc != null ? participant.PreviousEmploymentNoc.ToString() : string.Empty},
			};
