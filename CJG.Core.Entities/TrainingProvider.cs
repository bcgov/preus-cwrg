	...
	public class TrainingProvider : EntityBase
	{
		...
		public TrainingProgram TrainingProgram
		{
			get
			{
				...
				return TrainingPrograms.FirstOrDefault() ?? OriginalTrainingProvider?.TrainingProgram;
			}
		}
		...
		public TrainingProvider ApprovedTrainingProvider
		{
			get
			{
				...
				if (OriginalTrainingProviderId == null)
				{
					// Get the currently validated training provider.
					var validated = RequestedTrainingProviders.OrderByDescending(tp => tp.DateAdded).FirstOrDefault(tp => tp.TrainingProviderInventoryId != null && tp.TrainingProviderState == TrainingProviderStates.Complete)?.ApprovedTrainingProvider;
					// If there is not a validated training provider, get the most recent requested one.
					return validated ?? RequestedTrainingProviders.OrderByDescending(tp => tp.DateAdded).FirstOrDefault(tp => tp.TrainingProviderInventoryId == null && tp.TrainingProviderState == TrainingProviderStates.Complete)?.ApprovedTrainingProvider ?? this;
				}
				else
				{
					// Get the currently validated training provider.
					var validated = OriginalTrainingProvider.RequestedTrainingProviders.OrderByDescending(tp => tp.DateAdded).FirstOrDefault(tp => tp.TrainingProviderInventoryId != null && tp.TrainingProviderState == TrainingProviderStates.Complete);
					// If there is not a validated training provider, get the most recent requested one.
					return validated ?? OriginalTrainingProvider.RequestedTrainingProviders.OrderByDescending(tp => tp.DateAdded).FirstOrDefault(tp => tp.TrainingProviderInventoryId == null && tp.TrainingProviderState == TrainingProviderStates.Complete) ?? this;
				}
			}
		}
		...
		public TrainingProvider RequestedTrainingProvider
		{
			get
			{
				if (OriginalTrainingProviderId == null)
				{
					var requested = RequestedTrainingProviders.OrderByDescending(tp => tp.DateAdded).ThenByDescending(tp => tp.Id).FirstOrDefault(tp => tp.TrainingProviderState.In(TrainingProviderStates.Requested, TrainingProviderStates.RequestApproved, TrainingProviderStates.RequestDenied) && tp.DateAdded >= DateAdded);
					return requested?.RequestedTrainingProvider ?? requested;
				}
				else
				{
					var requested = OriginalTrainingProvider.RequestedTrainingProviders.OrderByDescending(tp => tp.DateAdded).ThenByDescending(tp => tp.Id).FirstOrDefault(tp => tp.TrainingProviderState.In(TrainingProviderStates.Requested, TrainingProviderStates.RequestApproved, TrainingProviderStates.RequestDenied) && tp.DateAdded >= DateAdded);
					if (requested?.Id == Id) return null;
					return requested?.RequestedTrainingProvider ?? requested;
				}
			}
		}
		...
		public TrainingProvider(TrainingProvider trainingProvider)
		{
			...
			OriginalTrainingProviderId = original.Id;
			OriginalTrainingProvider = original;
			TrainingProviderState = TrainingProviderStates.Requested;
			original.RequestedTrainingProviders.Add(this);
			EligibleCostId = original.EligibleCostId;
			EligibleCost = original.EligibleCost;
			GrantApplication = original.GrantApplication;
			GrantApplicationId = original.GrantApplicationId;
		}
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			var programs = context.Set<TrainingProgram>().Where(x => x.TrainingProviders.Any(y => y.Id == Id)).ToArray();
			if (GrantApplicationId.HasValue && GrantApplication == null)
				context.Set<GrantApplication>().Include(nameof(TrainingPrograms)).First(ga => ga.Id == GrantApplicationId);

			// This is ugly, but currently ensures that the appropriate grant application is loaded into memory.
			var originalTrainingProvider = context.Set<TrainingProvider>().Include(tp => tp.TrainingPrograms).FirstOrDefault(tp => tp.Id == OriginalTrainingProviderId);
			var grantApplicationId = GrantApplicationId ?? TrainingPrograms.FirstOrDefault()?.GrantApplicationId ?? (programs.Any() ? programs.First().GrantApplicationId : originalTrainingProvider?.GetGrantApplication()?.Id ?? 0);
			var grantApplication = (GrantApplication ?? TrainingPrograms.FirstOrDefault()?.GrantApplication) ?? context.Set<GrantApplication>().First(x => x.Id == grantApplicationId);

			// If TrainingOutsideBC there must be a BusinessCaseDocument.
			if (TrainingProviderState != TrainingProviderStates.Incomplete && TrainingOutsideBC && String.IsNullOrEmpty(BusinessCase) && (BusinessCaseDocumentId == null && BusinessCaseDocument == null))
			...
			var trainingProviderType = TrainingProviderType ?? context.Set<TrainingProviderType>().FirstOrDefault(x => x.Id == TrainingProviderTypeId);

			// for certain TrainingProviderTypes, there must be a proof of qualifications document and course outline
			var validatingTrainingProvider = entry.State == EntityState.Modified && (int?)entry.OriginalValues[nameof(TrainingProviderInventoryId)] != TrainingProviderInventoryId;
			if (!validatingTrainingProvider
				&& TrainingProviderState != TrainingProviderStates.Incomplete
				&& (
			...
			{
				if (ProofOfQualificationsDocument == null && ProofOfQualificationsDocumentId == null && trainingProviderType.ProofOfInstructorQualifications == 1)
					yield return new ValidationResult("You must provide proof of qualifications.", new[] { nameof(ProofOfQualificationsDocument) });

				if (CourseOutlineDocument == null && CourseOutlineDocumentId == null && trainingProviderType.CourseOutline == 1)
			...
			if (entry.State == EntityState.Modified)
			{
				...
				if (isExternalUser && EligibleCostId.HasValue && grantApplication.ApplicationStateInternal.In(ApplicationStateInternal.Draft, ApplicationStateInternal.ApplicationWithdrawn))
				{
					var eligibleCost = context.Set<EligibleCost>().Include(ec => ec.EligibleExpenseType).FirstOrDefault(ec => ec.Id == EligibleCostId.Value);
					if (!eligibleCost?.EligibleExpenseType?.IsActive ?? false)
					{
						yield return new ValidationResult($"The service category '{eligibleCost.EligibleExpenseType.Caption}' is no longer available, please select another.", new[] { nameof(EligibleCostId) });
					}
				}
			}
