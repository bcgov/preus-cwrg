...
namespace CJG.Core.Entities
{
	...
	public class GrantApplication : EntityBase
	{
		...
		/// <summary>
		/// get/set - The start date of the training program. Also known as "Delivery Start Date".
		/// </summary>
		[DateTimeKind(DateTimeKind.Utc)]
		...
		/// <summary>
		/// get/set - The end date of the training program. Also known as "Delivery End Date".
		/// </summary>
		[DateTimeKind(DateTimeKind.Utc)]
		...
		/// <summary>
		/// When was the application returned to draft state
		/// </summary>
		[DateTimeKind(DateTimeKind.Utc)]
		[Column(TypeName = "DATETIME2")]
		public DateTime? ReturnedToDraft { get; set; }
		...
		/// <summary>
		/// get/set - The Grant ApplicationEvaluation record.
		/// </summary>
		[ForeignKey(nameof(GrantApplicationEvaluationId))]
		...
		/// <summary>
		/// get/set - Foreign key to the Proof of Payment record.
		/// </summary>
		public int? ProofOfPaymentId { get; set; }

		/// <summary>
		/// get/set - The Proof of Payment record.
		/// </summary>
		[ForeignKey(nameof(ProofOfPaymentId))]
		public virtual ProofOfPayment ProofOfPayment { get; set; }

		/// <summary>
		/// get/set - Foreign key to the Attestation record.
		/// </summary>
		...
		[NotMapped]
		public DateTime TrainingStartDate => this.GetTrainingStartDate();
		...
		/// <summary>
		/// get - All of the evaluation notes associated with this application.
		/// </summary>
		public virtual ICollection<GrantApplicationEvaluationNote> EvaluationNotes { get; set; } = new List<GrantApplicationEvaluationNote>();
		...
		/// <summary>
		/// get - All of the completed tasks associated with this application.
		/// </summary>
		public virtual ICollection<GrantApplicationTask> GrantApplicationTasks { get; set; } = new List<GrantApplicationTask>();
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			// This is done to stop errors from being thrown when EF entities are used in ViewModels.
			if (entry == null)
				yield break;
			...
			// If the user editing is not external, allow for a relaxed start date
			if (!isExternalUser)
			{
				...
				hasValidStartDate = this.HasValidStartDate(trainingPeriodStartDate, maxDates.Item1);
			}
			...
			if (entry.State == EntityState.Added)
			{
				...
				if (!hasValidStartDate)
					yield return new ValidationResult($"Your project start date must fall in the period '{trainingPeriodStartDate.ToLocalMorning():yyyy-MM-dd}' to '{trainingPeriodEndDate.ToLocalMidnight():yyyy-MM-dd}' " +
			...
			else if (entry.State == EntityState.Modified)
			{
				...
				var validStatesForExternalValidation = new List<ApplicationStateInternal>
				{
				    ...
				    && !ApplicationStateInternal.In(ApplicationStateInternal.ApplicationWithdrawn, ApplicationStateInternal.Unfunded, ApplicationStateInternal.ApplicationDenied))
				{
					...
					var eligibleCosts = context.Set<EligibleCost>()
						...
						.Include(m => m.Breakdowns)
						.Include(m => m.TrainingProviders)
