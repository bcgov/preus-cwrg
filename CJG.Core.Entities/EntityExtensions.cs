...
namespace CJG.Core.Entities
{
	...
	{
		private static readonly ConcurrentDictionary<KeyValuePair<Type, string>, MethodInfo> _methods = new ConcurrentDictionary<KeyValuePair<Type, string>, MethodInfo>();

		private static readonly Dictionary<object, Dictionary<string, object>> PropertyValues = new Dictionary<object, Dictionary<string, object>>();
		...
		public static bool CopyPropertiesTo<TEntity>(this TEntity source, TEntity target, Func<TEntity, object> properties) where TEntity : class
		{
			...
			var sourceProperties = typeof(TEntity).GetProperties().Where(x => x.CanRead && (includedProperties.Contains(x.Name))).ToArray();
		...
		{
			return entity.GetKeys().Select(p => p.GetValue(entity)).ToArray();
		}
