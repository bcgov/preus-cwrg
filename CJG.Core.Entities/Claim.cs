...
namespace CJG.Core.Entities
{
	...
	public class Claim : EntityBase
	{
		...
		/// <summary>
		/// get/set - The Claim Evaluation record.
		/// </summary>
		[ForeignKey(nameof(ClaimEvaluationId))]
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			else if (entry.State == EntityState.Modified)
			{
				...
				var originalVersion = (int)entry.OriginalValues[nameof(ClaimVersion)];
				if (originalVersion != ClaimVersion)
				...
				var originalState = (ClaimState)entry.OriginalValues[nameof(ClaimState)];
				if (originalState.In(ClaimState.Incomplete, ClaimState.Complete) && ClaimState == ClaimState.Unassessed && (!DateSubmitted.HasValue || DateSubmitted.Value.Date < AppDateTime.UtcNow.Date))
				...
				var hasStateChanged = ClaimState != originalState;
				if (hasStateChanged)
				{
			...
			if (ClaimTypeId == ClaimTypes.SingleAmendableClaim &&
				...
				&& (participantsWithCostsAssigned == 0
					|| participantsWithoutCostsAssigned > 0))
			...
			if (EligibleCosts != null && EligibleCosts.Any())
			{
				...
				var maxParticipantReimbursement = (
					from pc in EligibleCosts.SelectMany(ec => ec.ParticipantCosts)
					group pc by pc.ParticipantFormId into g
					let sum = g.Sum(gpc => gpc.ClaimReimbursement)
					where sum > GrantApplication.MaxReimbursementAmt
					select sum).Count();

				var maxParticipantAssessedReimbursement = (
					from pc in EligibleCosts.SelectMany(ec => ec.ParticipantCosts)
					group pc by pc.ParticipantFormId into g
					let sum = g.Sum(gpc => gpc.AssessedReimbursement)
					where sum > GrantApplication.MaxReimbursementAmt
					select sum).Count();
