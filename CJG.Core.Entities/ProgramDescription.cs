...
namespace CJG.Core.Entities
{
	...
	public class ProgramDescription : EntityBase
	{
		...
		/// </summary>
		[Required(AllowEmptyStrings = false, ErrorMessage = "Project Description is required."), MaxLength(2000)]
		public string Description { get; set; }

	    /// <summary>
		/// get/set - A 'publicly available' description of the grant application training program. Not visible to applicant.
		/// </summary>
		[MaxLength(1000)]
		...
		/// <summary>
		/// get/set - The number of community representatives and employer letters submitted in support of this project.
		/// </summary>
		[Range(0, int.MaxValue, ErrorMessage = "Community representatives and employer letters submitted must be greater than or equal to 0.")]
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			context.Set<ProgramDescription>().Include(m => m.Communities).FirstOrDefault(pd => pd.GrantApplicationId == GrantApplicationId);

			// Can't change the state to complete unless all required values are entered.
			if (DescriptionState == ProgramDescriptionStates.Complete)
			{
				...
				if (GrantApplication.GrantOpening.GrantStream.GrantProgram.ProgramTypeId == ProgramTypes.WDAService)
				{
					...
					if (TargetNAICSId == null)
						yield return new ValidationResult("The target NAICS must be selected.", new[] { nameof(TargetNAICSId) });

					/// Target NOC required.
					if (TargetNOCId == null)
						yield return new ValidationResult("The target NOC must be selected.", new[] { nameof(TargetNOCId) });
				}
			}

			if (ApplicantOrganizationTypeId == ApplicantOrganizationType.OtherTypeId && string.IsNullOrEmpty(ApplicantOrganizationTypeInfo))
			{
				yield return new ValidationResult("Applicant Other Type is required.", new[] { nameof(ApplicantOrganizationTypeInfo) });
			}
