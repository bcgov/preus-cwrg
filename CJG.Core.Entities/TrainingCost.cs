...
namespace CJG.Core.Entities
{
	...
	public class TrainingCost : EntityBase
	{
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			var totalEstimatedReimbursement = EligibleCosts.Sum(ec => ec.EstimatedReimbursement);
			...
			if (GrantApplication.GetProgramType() != ProgramTypes.EmployerGrant && totalEstimatedReimbursement != TotalEstimatedReimbursement)
				yield return new ValidationResult($"The total estimated reimbursement is incorrect and should be {totalEstimatedReimbursement.ToString("c2")}", new[] { nameof(TotalEstimatedReimbursement) });
			...
			if (EligibleCosts.Any())
			{
				...
				if (EligibleCosts.Any(ec => ec.AddedByAssessor ? ec.AgreedMaxParticipants > AgreedParticipants : ec.EstimatedParticipants > EstimatedParticipants))
					yield return new ValidationResult($"The number of participants for one expense type cannot exceed the number of participants you entered in part 1, which was {EstimatedParticipants}.", new[] { nameof(EstimatedParticipants) });
				...
				if (!GrantApplication.ApplicationStateInternal.In(ApplicationStateInternal.Draft, ApplicationStateInternal.ApplicationDenied, ApplicationStateInternal.ApplicationWithdrawn, ApplicationStateInternal.Unfunded)
					...
					yield return new ValidationResult("The number of participants must be greater than 0.", new[] { nameof(EstimatedParticipants) });
				...
				var ids = EligibleCosts.Select(x => x.Id).ToList();
				...
				if (originalState != ApplicationStateInternal.ApplicationDenied && GrantApplication.ApplicationStateInternal != ApplicationStateInternal.ReturnedToAssessment)
				{
					...
					var eligibleExpenseTypeIds = GrantApplication.GrantOpening.GrantStream.ProgramConfiguration.EligibleExpenseTypes
						.Select(eet => eet.Id)
						.ToArray();
					...
					foreach (var eligibleCost in EligibleCosts)
					{
						...
						if (eligibleCost.EligibleExpenseType.ServiceCategory?.ServiceTypeId == ServiceTypes.SkillsTraining)
						{
							...
							{
								var eligibleCostBreakdowns = eligibleCost.Breakdowns;
								var serviceCategory = eligibleCost.EligibleExpenseType?.ServiceCategory;

								if (eligibleCostBreakdowns.Count(b => b.IsEligible) < serviceCategory?.MinPrograms)
								{
									yield return new ValidationResult($"{eligibleCost.EligibleExpenseType.Caption} expenses require at least {serviceCategory?.MinPrograms} components to be eligible.", new[] { nameof(EligibleCostBreakdown.IsEligible) });
								}
								else if (eligibleCostBreakdowns.Count(b => b.IsEligible && b.AssessedCost == 0) > 0)
								{
